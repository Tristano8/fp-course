...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = pa >>= \a -> list (ps *> pa) >>= \a' -> pure (a :. a')
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lps = satisfy (and . sequence lps)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lps = satisfy (or . sequence lps)
...
-- >>> isErrorResult (parse (betweenSepbyComma '[' ']' lower) "a]")
-- True
-- Parser a -> Parser s -> Parser (List a)
betweenSepbyComma ::
  Char
  -> Char
  -> Parser a
