...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus mv (ListZipper l a r) = ListZipper l (mv a) r
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a r) = let (y:.ys) = reverse (a :. r) in ListZipper ys y Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l a Nil) = let (y:.ys) = reverse (a:.l) in ListZipper Nil y ys
...
moveLeft (ListZipper (x:.xs) a ys) = IsZ (ListZipper xs x (a:.ys))
...
moveRight (ListZipper ys a (x:.xs)) = IsZ (ListZipper (a:.ys) x xs)
...
swapRight (ListZipper ys a (x:.xs)) = IsZ (ListZipper ys x (a:.xs)) 
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft c (ListZipper l a r) = ListZipper (a:.l) c r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight c (ListZipper l a r) = ListZipper l c (a:.r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fs f gs) (ListZipper l a r) = ListZipper (zipWith ($) fs l) (f a) (zipWith ($) gs r)
...
--  (ListZipper a -> Optional (b, ListZipper a))
--- -> ListZipper a
--- -> List b
...
instance Traversable ListZipper where
  traverse:: Applicative f =>
    ...
    -> ListZipper a
    -> f (ListZipper b)
  -- traverse afb (ListZipper l a r) = let a' = traverse afb l
  --                                       b' = afb a
  --                                       c' = traverse afb r
  --                                   in lift3 ListZipper a' b' c'
  -- The above compiles but is incorrect, as I think the applicative effects will not be run in the intended order
  traverse afb (ListZipper l a r) = let a' = traverse afb (reverse l)
                                        b' = afb a
                                        c' = traverse afb r
                                    in lift3 (ListZipper . reverse) a' b' c'
...
instance Traversable MaybeListZipper where
  ...
  traverse afb (IsZ z) = IsZ <$> traverse afb z
